...
const int KeyNum = 1; //キーボードエミュレーションを行うボタンの数を入力してください。最低でも1は必要です。
...
const int button[btnum] = {12}; //{}内にボタンを接続したピンの番号を入力してください。適当でいいので、必ずなにか入力してください。
const int btncord[btnum] = {HORN1}; //{}内に、上で設定した各ボタンの果たす機能を入力してください。名称での指定も可能です。詳しくは本スケッチに同梱の説明書をご覧ください。
const int KeyBtn[KeyNum] = {12}; //{}内に、上で設定した各キーボタンの果たす機能を入力してください。詳しくは本スケッチに同梱の説明書をご覧ください。
